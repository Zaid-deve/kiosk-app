...
if ($token) {
    try {
        ...
        if ($decoded && $decoded->sub) {
            ...
            $errors[] = 'User must be logged in to mark order received, please log in!';
        }
    } catch (Exception $e) {
...
$qry = $conn->query("UPDATE orders SET status = 'received' WHERE order_id = '$orderId' AND (user_id = '$userId' OR guest_id = '$guestId')");
if ($qry) {
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Failed to mark received order !'
    ]);
}
